...
| v0.5-lsb-bytes-auto-run | **架构升级** (per Owner 06-21 12:40 拍板, 把 lsb_bytes_extract 接进 auto-run 池兜底 zsteg 漏报): 新建 `tools/shared/lsb_bytes_extract_adapter.py:LsbBytesExtractAdapter` (包 LSBBytesExtractAction, 默认 12 组合 RGB×bit 0/7×row/col×MSB, ~5s/张图, auto-run 拖图片自动跑) + 双注册 (`tools/shared/__init__.py` + `tools/__init__.py`, per automisc-tool-registration memory) + `auto_runner.py:FIND_SUSPICIOUS_PICTURE_TOOLS` 加 lsb_bytes_extract (6→7 tools) + `menu_dock.py:ADAPTER_TOOLS` + TOOL_CATEGORIES 加显示 "🎨 LSB 字节流抽取 (auto-run 同款, 12 组合)";不动 LSBBytesExtractAction 核心, 不接 magic_sniffer (decoder 不混进 Action 链, per v0.5-lsb-byte-stream-extract §3.1);+9 单测 (adapter 注册 / 12 组合跑通 / SP 累积 / 文件不存在 graceful / 全失败 graceful / GUI 集成);+1 个 test_main_window.py + test_find_suspicious_runner.py 硬编码断言同步 (56→57 tool / 71→72 渲染 / 6→7 picture pool);不破坏 v0.5-lsb-bytes-gui GUI user-controlled 入口 (互补关系) | ✅ done | (main `2025a51`, 11 files +709/-12, +9 单测 9/9 passed, unit 1045 passed 0 新失败, GUI 集成 57 passed + 2 skipped) | [`upgrade/v0.5-lsb-bytes-auto-run.md`](upgrade/v0.5-lsb-bytes-auto-run.md) |

...
| v0.5-lsb-extract-output-bytes | **bug 修复** (per Owner 2026-06-21 22:55 实战反馈 + v0.5-train-011 实战): 修 `core/actions/lsb_extract.py:142 + 232-239` 写文件 — suffix 写死 `.txt` → 改 magic 判定 (89 50 4E 47 = .png / 50 4B = .zip / 03 f3 0d 0a = .pyc 等, 复用 `lsb_detect._detect_file_header_hex` 25+ magic 库) + `write_text` (UTF-8 decode) → `write_bytes` (二进制) + fallback `.bin` 默认;Q1=A 复用 lsb_detect magic / Q2=A 修 lsb_extract.py 保留 zsteg-based / Q3=A fallback .bin;新建 `tests/unit/core/actions/test_lsb_extract_output.py` ≥ 6 cases (text 分支 PNG/ZIP/PYC + fallback .bin + file 分支正确 + write_bytes 后 `file` 命令识别为实际类型);smoke: 工具栏跑 steg.png → 输出 `<stem>__lsb.png` (有效 PNG, `file` 命令识别 `PNG image data`);**不动 lsb_bytes_extract.py** (v0.5-lsb-byte-stream-extract 仍 .bin, 本 spec 范围外, 后续 v0.5+ 实战触发再统一) | 🔄 approved | (无 commit, 待实施) | [`upgrade/v0.5-lsb-extract-output-bytes.md`](upgrade/v0.5-lsb-extract-output-bytes.md) |

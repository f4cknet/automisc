...
| v0.5-lsb-byte-stream-extract | **架构升级** (B+C 合并, per v0.5-train-009 触发): 新建 `core/actions/lsb_bytes_extract.py` + `LSBBytesExtractAction` (PIL/numpy 直接抽字节流, 通道 × bit × scan_order × byte_bit_order 4 参数 user-controlled) + 新建 `core/decoders/magic_sniffer.py` (decoder 类, 滑动窗口扫 offset 0~32 匹配 28+ 文件 magic, 解决 `router.detect_magic` 只看 offset 0 痛点) + 新 chain `lsb-bytes` (binwalk_extract → LSBBytesExtractAction → magic_sniffer) + GUI Run→Chain 菜单新增 lsb-bytes 入口 + 参数 dialog;不动现有 zsteg-based `LSBExtractAction` (回归 0 风险) | 🟡 design | (无 commit, 等 Owner 拍板 §7 Q1-Q6 后改 approved → 实施) | [`upgrade/v0.5-lsb-byte-stream-extract.md`](upgrade/v0.5-lsb-byte-stream-extract.md) |

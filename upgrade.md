...
| v0.5-pyc-decompiler-gui | **GUI 集成小迭代** (per Owner 06-21 11:54 "把这个放到 gui 左边工具栏中"): 把 `decoder:pyc_decompiler` 加到 `gui/menu_dock.py:TOOL_CATEGORIES` 新分类 "🐍 反编译工具" + ACTION_DISPLAY_NAMES 显示名 "🐍 Pyc 反编译 (默认 Python 2)" (per Owner 决策 "大多数 pyc 反编译时 python2");不动 `pyc_decompiler.py` 核心逻辑 (默认走 uncompyle6 已实现);+5 GUI 集成测试 (ToolMenuDock 渲染 + click callback + run 验证) | ✅ done | (main TBD commit, 5 files +165/-3, +5 集成测试 5/5 passed, unit 1036 passed 0 新失败) | [`upgrade/v0.5-pyc-decompiler-gui.md`](upgrade/v0.5-pyc-decompiler-gui.md) |

...
| v0.5-lsb-detector | **架构升级** (per v0.5-train-010 触发, Owner 06-21 21:44 ~ 22:00 Q1=A / Q2=A / Q3=inline / Q4=A + 细节 1=6 排列 / 细节 2=readonly 拍板): **auto-run readonly 智能 LSB 检测** (需求 1 + 需求 2 合并, **替代 zsteg** auto-run 位, 互补 v0.5-lsb-byte-stream-extract lsb_bytes_extract 不替代);需求 1 (12 组合 = RGB 6 排列 × row/col 2 scan, 各 bit 0 抽字节流, **不写文件** 纯 readonly): text 判定 (printable ASCII 32-126 全字符 → sev=5) + 文件头双机制判定 (hex magic **主** + `file` 命令**辅**, 复用 magic_sniffer 50+ entry → sev=5);需求 2 (R/G/B 各自完整 8 bit 拼 byte 字节流): entropy + unique count 跨通道比较, 异常 → sev=4 **info 概率** (per Owner "没有绝对性");新建 `core/actions/lsb_detect.py:LSBDetectAction` (~200 LOC) + `tools/steganography/image/lsb_detect_adapter.py` (~80 LOC, 双注册 per automisc-tool-registration 铁律) + `gui/auto_runner.py:FIND_SUSPICIOUS_PICTURE_TOOLS` **删 zsteg 加 lsb_detect** (仍 6 tools) + 10 单测 + 3 集成测试;**zsteg adapter 文件保留** (不删, 供未来复用);**符合 AGENTS §1 铁律 7** (auto-run 不写文件, 不触发下一步, 只写 journal SP, Owner 决策) | 🔄 approved | (无 commit, 待实施) | [`upgrade/v0.5-lsb-detector.md`](upgrade/v0.5-lsb-detector.md) |

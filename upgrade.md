...
| v0.5-pyc-magic-sniffer | **架构升级** (per v0.5-lsb-byte-stream-extract 后续, per Owner 06-21 11:40 Q1+Q2 拍板): (1) `core/decoders/magic_sniffer.py` 扩展 EXTENDED_MAGIC_SIGNATURES 50+ entry, **重点加 13+ Py2.x / Py3.x magic 变体** (Py2.4/2.5/2.6/2.7 + Py3.0/3.2/3.6/3.10/3.12), 解 N=NP 题 Py2.7 magic (62211=`03 f3 0d 0a`) 命中; (2) 新建 `core/decoders/pyc_decompiler.py` + `run_pyc_decompiler` (uncompyle6 Py2.x / decompyle3 Py3.x / dis fallback) + DecoderSpec 注册, CLI `automisc decode pyc_decompiler --file X`;不动 lsb_bytes_extract 路径;**0 新失败 baseline**, +40 单测 (30 magic_sniffer + 10 pyc_decompiler) | 🟡 design | (无 commit, 等 Owner 拍板 §7 Q1-Q6 后改 approved → 实施) | [`upgrade/v0.5-pyc-magic-sniffer.md`](upgrade/v0.5-pyc-magic-sniffer.md) |

...
| `core/decoders/pyc_decompiler.py` | **v0.5-pyc-magic-sniffer 能力 E + v0.5-pyc-decompiler-buttons 扩展**: Py2.x / Py3.x .pyc 文件反编译到 Python 源码 (uncompyle6 / decompyle3 / dis fallback), 输入 .pyc 路径 → 输出 source_code + 元数据 (magic_int / version / force_version / output_path); `force_version` 参数 (None=auto/2=py2/3=py3) 路由强制版本, 成功反编译时写 `<stem>__pyc[_pyN].py` 到 pyc 同目录 (per v0.5-output-samedir) | `run_pyc_decompiler`, `PycDecompileResult`, `_purpose_for_force` |

...
| `core/decoders/sparse_grid_restore.py` | **v0.5-sparse-grid-restore**: sparse (col_index, char) 2D 网格 → 等宽 ASCII 字符画 (auto-detect JSON / Python literal / raw text; CSR dense 化, last-wins 写入; max 100K cells); GUI "共享基础工具 (PR1)" 分类 '🧩 稀疏网格还原' (跟 file/strings/trid 同级); 不进 auto-run 池; 实战触发: Owner 2026-07-01 Pickle 反序列化 30×76 三态 grid → 直接出 ASCII 字符画看图 | `SparseGridError`, `SparseGridResult`, `render`, `restore`, `MAX_CELLS`, `MAX_INPUT_SIZE` |

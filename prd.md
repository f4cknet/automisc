...
| P6 | `v0.1.0b-PR8` | Misc/Brainteaser QR（zbar）| ✅ | 2h | **已完成**（commit `待定`，2026-06-13 17:48）；**依赖**：PR9；**6 关验收**：② ✅ `pytest tests/unit` 全过（**149 passed** = PR1 61 + PR2 14 + PR9 22 + PR3 10 + PR4 17 + PR5 10 + PR6 10 + PR8 5）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：`automisc run --tool zbar --file tests/fixtures/sample_qr_flag.png` 命中 flag [5] + barcode_text [1] + meta [1]；`--file sample_qr_url.png` 命中 barcode_url [2] + meta [1]；①/⑤/⑥ 待本地 commit + Owner push 后勾 |

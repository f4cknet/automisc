...
| `v0.1.0b-PR2` | **Stego/Image 主工具（zsteg + steghide adapter）**：实现 `tools/steganography/image/zsteg.py`（PNG/BMP LSB 全通道检测）+ `tools/steganography/image/steghide.py`（JPEG/BMP/WAV/AU 隐写提取/嵌入，需要口令）+ 单元测试。**依赖**：`tools/shared/` 6 个 adapter 已就绪（PR1）| 🔄 | 3h | **当前进行中任务**；按 `tools.md §3.5` + `Architecture.md §4.4`；**6 关验收**：① 代码合入 `feat/v0.1.0b-PR2-image-stego` 分支（不 push，per `AGENTS.md §2.5.1`）；② `pytest tests/unit -q` 全过（基线 61 + 新增 ≥12 个 = ≥73）；③ N/A（PR2 无 GUI / 集成）；④ 真实样本 smoke：用 steghide 嵌入 + zsteg 检测双向验证；⑤ Owner 自审（单 Owner 项目，本地 commit 后 Owner push + 开 PR）；⑥ 文档同步（本表 + `tools.md §3.5` + `Architecture.md §4.4`）|

...
| P1 | `v0.1.0b-PR9` | **Python 包基座补完**：pyproject.toml 已写但 `python -m automisc` 未验证；补 `src/automisc/__main__.py` 入口 + dev install smoke + console_script 跑通 | ✅ | 1.5h | **已完成**（commit `待定`，2026-06-13 15:13）；**依赖**：cleanup；**6 关验收**：② ✅ `pytest tests/unit` 全过（**97 passed** = PR1 61 + PR2 14 + PR9 22）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：`automisc run --tool strings --file tests/fixtures/sample_text.txt` 命中 `flag{smoke_test_pr9_xyz}` [5]；①/⑤/⑥ 待本地 commit + Owner push 后勾 |

...
| P9 | `v0.1.0b-gui` | **GUI 主窗口（PySide6 macOS only）**：`gui/main_window.py`（QMainWindow + 文件拖拽 + setAcceptDrops + dragEnterEvent/dropEvent 调 Core.route）+ `gui/menu_dock.py`（左 QDockWidget 22 adapter 按 6 类分类）+ `gui/output_view.py`（中央 QPlainTextEdit，ANSI 高亮 severity）+ `gui/journal_panel.py`（底 QTabWidget：output / journal / suspicious）+ `gui/runner.py`（QThread + signal/slot 跑 Core.run_tool 异步）+ `tests/integration/gui/`（pytest-qt 拖拽 + 菜单触发 + 输出区渲染）+ `pyproject.toml` 加 `PySide6>=6.6` (optional `[gui]` extra) | ⏳ | 8h | **依赖**：PR3~PR8 + encoders + PR9 全 ✅；**放最后**——所有 adapter 跑通再做 GUI 集成，避免改 22 个 adapter 时 GUI 测试跟着改；**6 关验收**草案：② ✅ `pytest tests/integration/gui` 全过（拖拽 + 菜单 + 输出 + journal 4 集成测试）；③ ✅ pytest-qt 跑通对应 GUI 集成测试；④ ✅ 真实样本 smoke：拖 `tests/fixtures/sample_text.txt` 进窗口 → Core.route → 显示"推荐 strings" + 点击 strings → 跑通 → output 区高亮 `flag{smoke_test_pr9_xyz} [5]` + journal 面板记 1 条 suspicious；⑤ Owner 自审；⑥ 文档同步（prd.md §4.1 + Architecture.md §2/§4.4 + AGENTS.md §8）|

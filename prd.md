...
| P4 | `v0.1.0b-PR5` | Misc/Archive（sevenz + unzip + john）| ✅ | 4h | **已完成**（commit `待定`，2026-06-13 17:05）；**依赖**：PR9；**6 关验收**：② ✅ `pytest tests/unit` 全过（**134 passed** = PR1 61 + PR2 14 + PR9 22 + PR3 10 + PR4 17 + PR5 10）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：`automisc run --tool sevenz --file tests/fixtures/sample_archive_pseudo.zip` 命中 `Wrong password` [4]（伪加密信号）；正常 zip 命中 file count [1]；john 列出 zip2john/rar2john capability；unzip 命中 file count；①/⑤/⑥ 待本地 commit + Owner push 后勾 |

...
| P4 | `v0.1.0b-PR5` | Misc/Archive（sevenz + unzip + john）| ✅ | 4h | **已完成**（commit `813f367`，2026-06-13 17:05）；**依赖**：PR9；**6 关验收**：② ✅ `pytest tests/unit` 全过（**134 passed** = PR1 61 + PR2 14 + PR9 22 + PR3 10 + PR4 17 + PR5 10）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：`automisc run --tool sevenz --file tests/fixtures/sample_archive_pseudo.zip` 命中 `Wrong password` [4]（伪加密信号）；正常 zip 命中 file count [1]；john 列出 zip2john/rar2john capability；unzip 命中 file count；⑥ ✅ 文档同步；①/⑤ 待 Owner push + PR 后勾 |
| P5 | `v0.1.0b-PR6` | Forensics/Log（grep + evtx_dump）| ✅ | 2.5h | **已完成**（commit `18688d5`，2026-06-13 17:15）；**依赖**：PR9；**6 关验收**：② ✅ `pytest tests/unit` 全过（**144 passed** = PR1 61 + PR2 14 + PR9 22 + PR3 10 + PR4 17 + PR5 10 + PR6 10）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：`automisc run --tool grep --file tests/fixtures/sample_auth.log` 命中 password [4]×3 + sudo [2] + secret [4] + hidden [2]（共 6 个 log_keyword 可疑点）；evtx_dump 暂未生成真实 evtx fixture（python-evtx 无 from_records API），但 error path 测试全过（empty file → cannot mmap；missing file → exit 1；corrupt file → 不 panic）；⑥ ✅ 文档同步；①/⑤ 待 Owner push + PR 后勾 |
| P6 | `v0.1.0b-PR8` | Misc/Brainteaser QR（zbar）| ✅ | 2h | **已完成**（commit `bf42cbc`，2026-06-13 17:48）；**依赖**：PR9；**6 关验收**：② ✅ `pytest tests/unit` 全过（**149 passed** = PR1 61 + PR2 14 + PR9 22 + PR3 10 + PR4 17 + PR5 10 + PR6 10 + PR8 5）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：`automisc run --tool zbar --file tests/fixtures/sample_qr_flag.png` 命中 flag [5] + barcode_text [1] + meta [1]；`--file sample_qr_url.png` 命中 barcode_url [2] + meta [1]；⑥ ✅ 文档同步；①/⑤ 待 Owner push + PR 后勾 |
| P7 | `v0.1.0b-PR7-envfix` | **前置环境修复（解决 vol.py blocker）**：vol2 留 v0.5+ docker；v0.1 adapter 默认走 vol3（`pip install volatility3`）。决策记录在 `docs/decisions/v0.1.0b-PR7-vol-environment.md` | ✅ | 1h | **已完成**（commit `79d4c1b`，2026-06-13 17:55）；**依赖**：PR9；**不写 adapter**，只解决"装得上"；⑥ ✅ 文档同步 |
| P7 | `v0.1.0b-PR7` | Forensics/Memory（vol.py adapter）| ✅ | 3h | **已完成并合 main**（squash merge commit `c682f5c` by f4cknet via PR #12, 2026-06-13 19:34）；**依赖**：PR7-envfix；**6 关验收**：② ✅ `pytest tests/unit` 全过（**153 passed** = PR1 61 + PR2 14 + PR9 22 + PR3 10 + PR4 17 + PR5 10 + PR6 10 + PR8 5 + PR7 4）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：`automisc run --tool vol --file /tmp/empty.vmem` 跑通 vol3 2.28.0（4 个 plugin：pslist/pstree/netscan/filescan 全跑）；⚠️ 真实 vmem fixture 未生成（体积大；v0.5+ 准备小型 test vmem）；⑥ ✅ 文档同步；①/⑤ ✅ Owner 自审 + 合入 main |
| P8 | `v0.1.0b-encoders` | **Encoding 自编写（9h Python 模块）**：`core/encoders/base.py`（base16/32/58/62/64/85/91/2048/32768/65536）+ `core/encoders/classical.py`（ROT13/47/18 + Caesar + Vigenère + Atbash + Pigpen + Keyboard Shift + Affine + Rail Fence）+ `core/encoders/custom.py`（BCD + IEEE 754 + UTF-16 endianness + Unicode Tags/Variation Selector + Multi-layer auto-decoder）+ 单元测试 | ✅ | 9h | **已完成并合 main**（squash merge commit `c94d5405` by f4cknet via PR #13, 2026-06-13 18:43）；**依赖**：PR9；非工具池层（per Architecture.md §4.4）；**6 关验收**：② ✅ `pytest tests/unit/core/encoders` 全过（**46 passed in 0.09s** = base 21 + classical 14 + custom 13）；③ N/A（无 GUI）；④ ✅ 真实样本 smoke：base64 roundtrip / Vigenère `LEMON` / Affine `(5,8)` / Rail Fence 经典 3-rail 例；⑥ ✅ 文档同步；①/⑤ ✅ Owner 自审 + 合入 main；**注**：commit subject 含 GitHub 自动拼接的"Feat/v0.1.0b encoders"前缀（PR 标题 placeholder 残留），Owner 决定接受脏 subject 不 amend force push |
| P9 | `v0.1.0b-gui` | **GUI 主窗口（PySide6 macOS only）**：`gui/main_window.py`（QMainWindow + 文件拖拽 + setAcceptDrops + dragEnterEvent/dropEvent 调 Core.route）+ `gui/menu_dock.py`（左 QDockWidget 22 adapter 按 8 类分类）+ `gui/output_view.py`（中央 QPlainTextEdit，severity 颜色高亮）+ `gui/journal_panel.py`（底 QDockWidget 累积 suspicious_points + time/tool/file/sev/kind/value 6 列）+ `tests/integration/gui/`（pytest-qt 15 集成测试：main_window 3 + menu_dock 4 + output_view 2 + journal_panel 3 + end-to-end 3）+ `pyproject.toml` 加 `PySide6>=6.6` (dependencies) | ✅ | 8h | **已完成**（commit `2a4fbe0` by mavis, main 上, 2026-06-13 20:00）；**依赖**：PR3~PR8 + encoders + PR9 全 ✅；**6 关验收**：② ✅ `pytest tests/integration/gui` 全过（**15 passed in 0.63s**）；③ ✅ pytest-qt 跑通（`QT_QPA_PLATFORM=offscreen`）；④ ✅ 真实样本 smoke：`TestEndToEnd::test_run_tool_against_sample` 拖 `sample_text.txt` + 选 strings → output 显示 `flag{smoke_test_pr9_xyz}` + journal 累积 1 条；⑤ ✅ Owner 自审（"完全信任 AI"决策）；⑥ ✅ 文档同步；**注**：v0.1 简化未写 `runner.py`（QThread 异步）—— 同步跑工具对短任务够用；v0.5+ 加 runner.py |

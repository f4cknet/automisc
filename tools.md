...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-06-30 | **2.18** | **v0.5-qemu-img-remove 反转 v0.5-qemu-img-adapter 决策** (per Owner 2026-06-30 22:16 drop `flag.vmdk` 实战 + 反馈 "全局删除 qemu_img，我现在使用新的7z 可以解压vmdk了"): (1) **实测 7z 23.01 支持 vmdk** (`7z x flag.vmdk` 解出 4 项: key_part_one/ + key_part_two/ + _NUL + where_is_flag_part_two.txt, per `v0.5-train-019-7z-version-vmdk.md`); (2) **删除 2 adapter** `qemu_img` (info 探测, 121 行) + `qemu_img_extract` (convert 写盘, 110 行) + `_binary_not_found_result` helper; (3) **删除 extend-tools/fix_qemu_img_setup.ps1** (56 行 NSIS 静默装 helper); (4) **清理 6 处引用**: menu_dock (Misc/Archive 5→3 entries + ADAPTER_TOOLS 删 + ACTION_DISPLAY_NAMES 删) + auto_runner (FIND_SUSPICIOUS_BINARY_TOOLS 5→4 tools) + archive/__init__.py (2 imports + 注释); (5) **删除 test_qemu_img.py** (370 行) + **改 test_paths.py** 删 TestResolveToolBinaryQemuImg; (6) **extend-tools 清理**: manifest.yaml version 1.2→1.3 + 删 qemu-img 条目 + 7zip notes doc-fix; install.ps1 删 qemu_img_setup_silent post_extract switch; (7) **tools.md 删** §3.9 qemu-img 行 + §6.1 P0 #23 #24 行 + **加 changelog 2.18**; (8) **doc fix 3 处**: `v0.5-train-018` 顶部加 "Doc-fix 2026-06-30" 段 + `v0.5-7z-layout-migrate` §8 关联改 + `extend-tools/manifest.yaml` 7zip notes 改. 决策反复合理 (两次都有实战依据: 7z 26.02 不支持→装 qemu-img, 7z 23.01 支持→删 qemu-img), 留 history 即可 (per AGENTS §2.4 不删 done 迭代). +8 commits 单 PR. 详见 [`upgrade/v0.5-qemu-img-remove.md`](./upgrade/v0.5-qemu-img-remove.md) + [`upgrade/v0.5-train-019-7z-version-vmdk.md`](./upgrade/v0.5-train-019-7z-version-vmdk.md). |
| 2026-06-30 | **2.17** | **qemu-img 11.0.50 实装 + e2e 激活 + fix_qemu_img_friendly_error 实战闭环** (per Owner 2026-06-30 00:15 手工装 `extend-tools/bin/win-x64/qemu/qemu-img.exe` v11.0.50): (1) e2e test_qemu_img.py 13/13 PASS (从 12+1 skip → 13 passed, fixture 检测 `real_binary` 跳过 autouse mock 走真 CLI; HAS_QEMU_IMG 改走 `resolve_tool_binary` 4 级 fallback 而非 `shutil.which`); (2) 真 binary 实战测 `qemu-img info <fake.bin>` exit 0 输出 `file format: raw` ✓; (3) tools.md §3.9 + §6.1 P0 表 #23 #24 路径/版本同步 (C:\Program Files\qemu\ → extend-tools/bin/win-x64/qemu\ + v9.1.0 → v11.0.50); (4) install.ps1 §82 URL `qemu-w64-setup-2025.05.12.exe` 失效 (404, Stefan Weil 站点 2026-04-22 后改 11.0.0 命名) → Owner 手工装路径作 fallback, install.ps1 后续 v0.5+ 评估修 URL (实战 ≥3 道同类再升架构). 详见 [`upgrade/fix_qemu_img_friendly_error.md`](./upgrade/fix_qemu_img_friendly_error.md). |
| 2026-06-29 | **2.16** | **v0.5-qemu-img-adapter 落地 + 升架构反转 v0.5-qemu-img-extend-tools §2.4 决策** (per Owner 2026-06-29 23:23 反馈"工具栏怎么没有 qemu-img 工具" + 拍 B 升架构): (1) **2 adapter 新建** (per v0.5-sevenz-extract 模式): `qemu_img` (info 探测, 可挂 auto-run, SP vdisk_format sev=3) + `qemu_img_extract` (convert 写盘, GUI 工具栏, SP vdisk_extracted sev=5, 不挂 auto-run per 铁律 7); (2) **双注册** (per archive __init__.py 模式); (3) **menu_dock** "Misc/Archive" 加 2 项: 🖼️ qemu-img 探测 (info) + 🖼️ qemu-img 转换 (vmdk→raw); (4) **auto-run pool** FIND_SUSPICIOUS_BINARY_TOOLS 加 qemu_img (探测归探测); (5) **v0.5-qemu-img-extend-tools §2.4 反转** "不建 adapter" → "建 (per v0.5-qemu-img-adapter 实战需求强)", owner memory "✅ 已装 但**没** GUI 入口" 模式. +9 单测 test_qemu_img.py 全过. 实战 1 道同类**破例**升架构 (per §5.2 owner 拍板可破例, 跟 v0.5-sevenz-extract 类似先例). 详见 [`upgrade/v0.5-qemu-img-adapter.md`](./upgrade/v0.5-qemu-img-adapter.md). |
| 2026-06-29 | **2.15** | **v0.5-fix-find-suspicious-race-condition + v0.5-qemu-img-extend-tools + v0.5-sevenz-extract doc-fix 落地** (per Owner 2026-06-29 22:30 race condition 实战 + 22:55 v0.5-train-018 vmdk 实战 + 22:57/22:58 拍板 1A/2Y/3 装): (1) **race condition fix**: `tools/base.py` `_run_subprocess` 改用 `subprocess.Popen` 持有 `_current_proc` handle + `_terminate_current_proc()` 强 kill (terminate 1s → kill 兜底); `core/orchestrator.py` 加 `_last_adapter` 引用 + `kill_last_subprocess()` 给 main_window 调; `gui/main_window.py` `_on_new_file_selected` 拖新文件时调 `core.kill_last_subprocess()` 强清旧 (旧 steghide 30s timeout 段不再写入新 output 区). +8 单测 test_race_condition_kill.py 全过. (2) **sevenz_extract doc-fix** (0 代码, per v0.5-train-018 实战反馈): spec 头部加 "📝 Doc-fix 2026-06-29" 段, 标记"p7zip 17.05 支持 vmdk"写错, 7z 26.02 实测不支持 vmdk/vhd/iso/ova, 实战走 `qemu-img convert` 路径. (3) **qemu-img extend-tools 装**: `manifest.yaml` 加 qemu-img 块 (QEMU Win NSIS installer, qemu.weilnetz.de); `install.ps1` Stage 1 加 `qemu_img_setup_silent` post_extract (调 `setup.exe /S` 静默装, **不**复制 500MB+ 到 extend-tools, 走 `C:\Program Files\qemu\` + PATH); +2 单测 test_paths.py `TestResolveToolBinaryQemuImg`. 详见 [`upgrade/v0.5-fix-find-suspicious-race-condition.md`](./upgrade/v0.5-fix-find-suspicious-race-condition.md) + [`upgrade/v0.5-qemu-img-extend-tools.md`](./upgrade/v0.5-qemu-img-extend-tools.md) + [`upgrade/v0.5-train-018-vmdk-extract.md`](./upgrade/v0.5-train-018-vmdk-extract.md). |
| 2026-06-29 | **2.14** | **v0.5-train-018 训练日志归档** (per Owner 2026-06-29 22:55 flag.vmdk 实战触发): flag.vmdk (面具下的flag writeup) 实战暴露 2 bug — 7z 26.02 不支持 vmdk (per sevenz_extract spec 写错) + foremost 1.5.7 Win length overflow bug. 实战答案: `qemu-img convert -f vmdk -O raw flag.vmdk flag.raw` → foremost 雕 raw → 2 flag 文件 (NULL brainfuck + flag_part_two.txt Ook!). 架构判定: 实战 1 道同类, 都不升架构 (per AGENTS §5.2). 详见 [`upgrade/v0.5-train-018-vmdk-extract.md`](./upgrade/v0.5-train-018-vmdk-extract.md). |
| 2026-06-29 | **2.13** | **v0.5-extend-tools-subdir-flexible 落地 + paths.py 异名 subdir fallback + xxd/steghide_audio adapter 修通** (per Owner 2026-06-29 22:00/22:02/22:06 反馈 + 实测 `vim92\xxd.exe` 跑通 hex dump): (1) `tools/paths.py:resolve_tool_binary` 4 级 subdir fallback 加通配扫 `bindir/*/<name>.exe` 1 层 (不递归 + 跳 .git 隐藏), 支持 `vim92/xxd.exe` / `vim92/diff.exe` 等异名 subdir 部署 (旧 `steghide/steghide.exe` 兼容路径保留); (2) `xxd.py` / `steghide_audio.py` adapter 改走 `resolve_tool_binary` (补救 v0.5-platform-extend-tools PR2 漏改 12 个 adapter 里的 2 个 Win 装了工具); (3) `manifest.yaml` vim92.notes 末尾加 1 行 annotation (vim92 块已存在 Owner v0.5-zbar-windows-install 加过, 不重加 3 块). 回归: xxd 4 FAIL → 6 PASS + steghide_audio 1 FAIL → 0 FAIL. 详见 [`upgrade/v0.5-extend-tools-subdir-flexible.md`](./upgrade/v0.5-extend-tools-subdir-flexible.md). |

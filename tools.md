...
| **qemu-img** | ✅ | `C:\Program Files\qemu\qemu-img.exe` (NSIS setup 装, PATH 自动注册) | 虚拟磁盘探测/转换 (vmdk/qcow2/raw/info/convert) | v0.5-qemu-img-adapter (Owner 23:23 拍 B 升架构, 2 adapter: qemu_img info + qemu_img_extract convert) |
...
| 23 | **qemu_img (info)** | Misc/Archive（虚拟磁盘探测）| ✅ 已装（`C:\Program Files\qemu\qemu-img.exe`，QEMU Win v9.1.0 NSIS installer 静默装 + 自动加 PATH，per [`upgrade/v0.5-qemu-img-extend-tools.md`](./upgrade/v0.5-qemu-img-extend-tools.md)；可挂 auto-run） |
| 24 | **qemu_img_extract (convert)** | Misc/Archive（虚拟磁盘写盘） | ✅ 已装（同上 binary；GUI 工具栏入口，不挂 auto-run per AGENTS §1 铁律 7；写盘 SP vdisk_extracted sev=5，per [`upgrade/v0.5-qemu-img-adapter.md`](./upgrade/v0.5-qemu-img-adapter.md)） |

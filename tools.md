...
| **qemu-img** | ✅ | `extend-tools/bin/win-x64/qemu/qemu-img.exe` (Owner 2026-06-30 00:15 手工装 v11.0.50; 异名 subdir fallback 走 paths.py 第 4 级 + 不需加 PATH) 或 `C:\Program Files\qemu\qemu-img.exe` (NSIS install.ps1 装) | 虚拟磁盘探测/转换 (vmdk/qcow2/raw/info/convert) | v0.5-qemu-img-adapter (Owner 23:23 拍 B 升架构, 2 adapter: qemu_img info + qemu_img_extract convert) |
...
| 23 | **qemu_img (info)** | Misc/Archive（虚拟磁盘探测）| ✅ 已装（`extend-tools/bin/win-x64/qemu/qemu-img.exe` QEMU Win v11.0.50 Owner 2026-06-30 00:15 手工装 + `paths.py` 第 4 级异名 subdir fallback 走通；e2e test_qemu_img.py 13/13 PASS；可挂 auto-run per [`upgrade/v0.5-qemu-img-extend-tools.md`](./upgrade/v0.5-qemu-img-extend-tools.md) + [`v0.5-qemu-img-adapter.md`](./upgrade/v0.5-qemu-img-adapter.md)） |
| 24 | **qemu_img_extract (convert)** | Misc/Archive（虚拟磁盘写盘） | ✅ 已装（同上 binary；GUI 工具栏入口，不挂 auto-run per AGENTS §1 铁律 7；写盘 SP vdisk_extracted sev=5） |
